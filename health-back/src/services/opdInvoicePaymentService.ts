...
import { opdOutstandingInvoiceTextSearchWhere } from "../lib/searchWhere";
...
export type OutstandingOpdInvoiceRow = {
  id: string;
  createdAt: string;
  balanceDue: string;
  totalAmount: string;
...
export async function listOutstandingOpdInvoices(params: {
  skip: number;
  take: number;
  q?: string;
}): Promise<{ items: OutstandingOpdInvoiceRow[]; total: number }> {
  const base = {
    invoiceTypeLookup: { is: { lookupKey: "OPD" } },
    opdInvoice: { isNot: null },
    balanceDue: { gt: 0 },
  };
  const where = params.q?.trim()
    ? { AND: [base, opdOutstandingInvoiceTextSearchWhere(params.q)] }
    : base;
  ...
  const items = rows.map((r) => ({
    ...
    bookingId: r.opdInvoice?.bookingId ?? "",
    bookingScheduledDate: r.opdInvoice?.booking?.scheduledDate?.toISOString() ?? null,
    patientId: r.opdInvoice?.patientId ?? null,
    patientName: r.opdInvoice?.patient?.fullName ?? null,
  }));

  return { items, total };
}

export async function recordOpdInvoicePayment(params: {
  invoiceId: string;
  ...
  const result = await prisma.$transaction(async (tx) => {
    const invoice = await tx.invoice.findUnique({
      ...
      select: {
        ...
        paidAmount: true,
        balanceDue: true,
        opdInvoice: { select: { bookingId: true } },
      },
    });
    ...
    if (
      ...
      !invoice.opdInvoice?.bookingId
    ) {
      throw new Error("Invoice is not an OPD invoice");
    }
