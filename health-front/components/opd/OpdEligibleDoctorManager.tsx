...
export type OpdEligibleRow = {
  userId: string;
  isActive: boolean;
  user: StaffProfile;
};

export function OpdEligibleDoctorManager(props: {
  profiles: StaffProfile[];
  eligible: OpdEligibleRow[];
}) {
  ...
  async function setEligible(userId: string, isActive: boolean) {
    setBusyId(userId);
    try {
      const res = await fetch(`/api/opd/eligible-doctors/${encodeURIComponent(userId)}`, {
        method: "PUT",
      ...
      toast.success(isActive ? "OPD access updated" : "Marked inactive");
      router.refresh();
    } catch (e) {
      ...
      <table className="min-w-full border-collapse text-left text-sm">
        <thead>
          <tr className="border-b border-[var(--border)] bg-[var(--surface-2)] text-xs text-[var(--text-muted)]">
            ...
            <th className="px-3 py-2 font-medium">OPD eligible</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((p) => {
