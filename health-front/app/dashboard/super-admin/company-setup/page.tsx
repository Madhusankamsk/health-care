...
function fromSettings(settings: CompanySettings): FormState {
  return {
      ...
      try {
        ...
        if (!res.ok) {
          const body = (await res.json().catch(() => null)) as
            | { message?: string }
            | null;
          throw new Error(body?.message ?? "Failed to load company settings.");
        }
        const settings = (await res.json().catch(() => null)) as CompanySettings;
        if (!cancelled) setForm(fromSettings(settings));
      } catch (e) {
        if (!cancelled)
              ...
              <label className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white px-3 py-3 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300">
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      isSetupCompleted: e.target.checked,
                    }))
                  }
                />
                Setup completed
