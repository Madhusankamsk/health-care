...
export default async function OpdInvoicesPage({
  searchParams,
}: {
  ...
  const result = await backendJsonPaginated<OutstandingOpdInvoiceRow>(
    `/api/opd-invoices/outstanding?${pageQueryString(pageNum, DEFAULT_PAGE_SIZE, q)}`,
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card>
        <CrudToolbar
          title="OPD invoices"
          description="Outstanding OPD walk-in invoices generated when a consultation is completed."
        />
        <TableSearchBarUrlSync
          initialQuery={q}
          id="opd-invoices-search"
        ...
        {!result ? (
          ...
          <p className="text-sm text-(--text-secondary)">No outstanding OPD invoices.</p>
        ) : (
          <>
            ...
            <TablePaginationBarFromSearch
              ...
              total={result.total}
              pathname="/dashboard/payments/opd-invoices"
              q={q}
            />
          </>
        )}
